...
const fetchRecommendations = async (
    bookIds: number[],
    count: number
): Promise<IRecommendedBookWithTask[]> => {
    const response = await axios.post(
        `http://localhost:8000/recommendations/find_recommendations`,
        {
            ids: bookIds,
            k: count,
        }
...
const Recommendations = ({ selectedBooks }: IProps) => {
    ...
    const sortedRecommendations =
        data?.sort((a, b) => b.book.similarity - a.book.similarity) ?? [];
    ...
    return (
        <div className="mt-16 w-full mb-8">
            ...
            <Flex align="center" justify="center" gap="small">
                <InputNumber
                    addonBefore="Ilość rekomendacji: "
                    min={2}
                    max={10000}
                    defaultValue={5}
                    onChange={(value) => {
            ...
            {isSuccess && (
                <>
                    ...
                    >
                        <RecommendedBooks
                            recommendedBooks={sortedRecommendations}
                        />
                    </Flex>
                    <Flex
                        className="mt-4"
                    ...
                    >
                        <Text className="text-xl">
                            Ilość trafionych rekomendacji względem informacji z
                            Goodreads&nbsp;
                            {sortedRecommendations.reduce(
                                (acc, recommendation) =>
                                    acc +
                                    (recommendation.book.is_in_similar_books
                                        ? 1
                                        : 0),
                                0
                            )}
