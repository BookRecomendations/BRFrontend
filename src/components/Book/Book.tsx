...
const fetchBookStatus = async (id: string) => {
    const response = await fetch(`https://api.example.com/books/${id}/status`);
    return response.json();
}
...
const Book = ({book}: IProps) => {
    ...
    const {data: bookStatus} = useQuery({
        queryKey: ['bookStatus', bookStatusId],
        queryFn: () => fetchBookStatus(bookStatusId),
